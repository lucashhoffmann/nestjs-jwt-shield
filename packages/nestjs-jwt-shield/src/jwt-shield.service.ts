...
import {
  assertImpersonationLink,
  assertJwtShieldImpersonationClaims,
  createJwtShieldImpersonationClaims,
  type JwtShieldAssertImpersonationLinkOptions,
  type JwtShieldImpersonationClaims,
  type JwtShieldImpersonationParticipant,
  type JwtShieldImpersonationSession,
  type JwtShieldImpersonationVerifiedClaims,
  type JwtShieldSignImpersonationTokenInput,
  type JwtShieldSignImpersonationTokenOptions,
  type JwtShieldVerifyImpersonationSessionInput,
} from "./impersonation/jwt-shield-impersonation";
import type {
...
export class JwtShieldService<TClaims extends object = JwtShieldDefaultClaims> {
  ...
  async verifyAccessToken(
    token: string,
  ): Promise<JwtShieldVerifiedClaims<TClaims>> {
  ...
  >(
    ...
    signOptions: JwtShieldSignImpersonationTokenOptions = {},
  ): Promise<string> {
    const claims = createJwtShieldImpersonationClaims(input);

    return this.signAccessToken(
      claims as unknown as JwtShieldAccessTokenPayload<TClaims>,
      {
        accessTokenTtl:
          signOptions.impersonationTokenTtl ??
          this.options.impersonationTokenTtlSeconds,
      },
    );
  }

  async verifyImpersonationToken(
    token: string,
  ): Promise<
    JwtShieldImpersonationVerifiedClaims<JwtShieldVerifiedClaims<TClaims>>
  > {
    const claims = await this.verifyAccessToken(token);

    try {
      assertJwtShieldImpersonationClaims(claims);
    } catch (error) {
  ...
  async verifyImpersonationSession<
    TActor extends object = JwtShieldVerifiedClaims<TClaims>,
    TImpersonation extends object = JwtShieldImpersonationVerifiedClaims<
      JwtShieldVerifiedClaims<TClaims>
    >,
  >(
    input: JwtShieldVerifyImpersonationSessionInput<TActor, TImpersonation>,
  ): Promise<JwtShieldImpersonationSession<TActor, TImpersonation>> {
    const primary = (await this.verifyAccessToken(
      input.primaryToken,
    )) as unknown as TActor;
    const impersonation = (await this.verifyImpersonationToken(
      input.impersonationToken,
    )) as unknown as TImpersonation;
  ...
  >(
    ...
    return createJwtShieldImpersonationClaims(input);
  }

  assertImpersonationLink<TActor extends object, TImpersonation extends object>(
    options: JwtShieldAssertImpersonationLinkOptions<TActor, TImpersonation>,
  ): void {
    assertImpersonationLink(options);
  }
  ...
  private validateApplicationClaims(
    ...
    if (!isRecord(claims)) {
      throw new JwtShieldInvalidClaimsError(
        "JWT claims schema must return an object.",
      );
    }
  ...
  private assertRegisteredClaims(
    payload: JWTPayload,
  ): JwtShieldRegisteredClaims {
    if (typeof payload.sub !== "string" || payload.sub.trim().length === 0) {
      throw new JwtShieldInvalidClaimsError(
        'JWT is missing required "sub" claim.',
      );
    }

    if (typeof payload.exp !== "number") {
      throw new JwtShieldInvalidClaimsError(
        'JWT is missing required "exp" claim.',
      );
    }

    if (typeof payload.iat !== "number") {
      throw new JwtShieldInvalidClaimsError(
        'JWT is missing required "iat" claim.',
      );
    }
    ...
    if (!hasAudience(payload.aud)) {
      throw new JwtShieldInvalidClaimsError(
        'JWT is missing required "aud" claim.',
      );
    }
...
function omitManagedRegisteredClaims(
  ...
  return Object.fromEntries(
    Object.entries(payload).filter(
      ([key]) => !MANAGED_REGISTERED_CLAIMS.has(key),
    ),
  );
}
